...
return function(_context, players: { Player }, _stampType: Stamps.StampType, stampId: string, stampTier: Stamps.StampTier | nil)
    ...
    for _, player in pairs(players) do
        local didNotOwnBefore = StampService.addStamp(player, stampId, stampTier)
        ...
        if didNotOwnBefore then
            ...
            output ..= (" > %s already owns the %q %s Stamp\n"):format(player.Name, stamp.DisplayName, tierSuffix)
        end
    end
